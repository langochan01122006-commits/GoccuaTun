...
import { getAllVotes, voteForCharacter, unvoteForCharacter, subscribeToVotes, signInWithGoogle, signInWithApple, handleRedirectResult, loginWithEmailPassword, registerWithEmailPassword, updateUserCustomProfile, logoutUser, auth } from "./firebase";
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md">
            ...
            >
              ...
              <div className="bg-[#180505] p-2 rounded-xl border border-amber-500/20 mb-4 text-[11px] text-amber-200/80 leading-relaxed text-center">
                🌐 <span className="font-bold text-amber-300">Đăng nhập Chuyển Hướng (Redirect):</span> Tự động chuyển trang tới Google/Apple ID. Nếu chạy trong khung xem thử (iframe), hãy <span className="text-amber-300 underline font-bold cursor-pointer" onClick={() => window.open(window.location.href, '_blank')}>Mở trang trong Tab Mới ↗</span> để trình duyệt chuyển hướng hoàn toàn!
              </div>
