...
import { getAllVotes, voteForCharacter, unvoteForCharacter, subscribeToVotes, signInWithGoogle, handleRedirectResult, loginWithEmailPassword, registerWithEmailPassword, updateUserCustomProfile, logoutUser, auth } from "./firebase";
...
export default function App() {
  ...
  useEffect(() => {
    // Process returning OAuth redirects
        ...
        {showAuthModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md">
            ...
            >
              ...
              {/* Tabs */}
              <div className="grid grid-cols-2 gap-2 mb-5 bg-[#180505] p-1.5 rounded-xl border border-amber-500/30">
                <button
                  type="button"
              ...
              <div className="mb-3">
                <button
                  ...
                  disabled={isAuthLoading}
                  className="w-full flex items-center justify-center gap-2 bg-white text-gray-800 hover:bg-gray-100 font-bold text-xs py-2.5 px-3 rounded-xl border border-gray-300 shadow transition cursor-pointer"
              ...
              {/* Fast Open Tab Option if in Iframe */}
              <div className="bg-[#180505] p-2 rounded-xl border border-amber-500/20 mb-4 text-[11px] text-amber-200/80 leading-relaxed text-center">
                🌐 <span className="font-bold text-amber-300">Đăng nhập Chuyển Hướng (Redirect):</span> Tự động chuyển trang tới Google. Nếu chạy trong khung xem thử (iframe), hãy <span className="text-amber-300 underline font-bold cursor-pointer" onClick={() => window.open(window.location.href, '_blank')}>Mở trang trong Tab Mới ↗</span> để trình duyệt chuyển hướng hoàn toàn!
              </div>

              <div className="relative flex items-center justify-center mb-5">
                <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-amber-500/30" /></div>
                <span className="relative px-3 bg-[#250808] text-xs text-amber-300/70 uppercase">Hoặc dùng Email & Mật Khẩu</span>
              </div>
